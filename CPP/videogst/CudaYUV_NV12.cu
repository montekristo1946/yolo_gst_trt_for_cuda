...
template<typename T>
static cudaError_t launchNV12ToRGB( void* srcDev, T* dstDev, size_t width, size_t height,size_t srcPitch, cudaStream_t stream )
{
    if( !srcDev || !dstDev )
...
cudaError_t CudaYUV_NV12::CudaNV12ToRGB( void* srcDev, uchar3* destDev, size_t width, size_t height,size_t srcPitch, cudaStream_t stream)
{
